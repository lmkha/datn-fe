...
export default function PlayListSelect(props: PlayListSelectProps) {
    ...
    const fetchData = async () => {
        if (!user?.id) return;
        const getAllPlaylistResult = await getAllPlaylistByUserId(user.id);
        const selectedPlaylist = getAllPlaylistResult.data.find((playlist: any) => playlist.id === props.playlistId);
        setState({
            ...state,
            allPlaylists: getAllPlaylistResult.data,
            selectedPlaylist: selectedPlaylist ? { id: selectedPlaylist.id, name: selectedPlaylist.name } : undefined,
        });
    };
